...
bool check_neigh(const Map& map, char c, uint y, uint x)
{
    ...
    if (x < w && y < h) {
        ...
        return (int)map[y][x] - (int)c <= 1;
    }
    return false;
}


// Use Dijkstra's algorithm to find minimum path cost from S to E.
uint32_t get_min_value(const Map& map)
{
    ...
    Vertex* p_start = nullptr, *p_end = nullptr;
    ...
    uint32_t min_value = p_end->value;
    delete[] graph;
    return min_value;
}
